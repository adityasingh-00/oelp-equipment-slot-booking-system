...
function StudentDetailf() {
  ...
  const [reason, setReason] = useState({ facultyIncharge: "" });
  ...
  if (rejnmessage) {
    for (let i = 0; i < rejnmessage.length; i++) {
      if (
        rejnmessage[i].jobno === parseInt(jobId) &&
        rejnmessage[i].supervisor
      ) {
        msg.push(rejnmessage[i].supervisor);
        break;
      }
    }
  }
  ...
  const deleteTimeSlots = (jobId) => {
    console.log("hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii");
    const reqTime = [];
    console.log("hii");
    if (students && popSlots) {
      let check = false;
      console.log("entered");

      for (let j = 0; j < popSlots.length; j++) {
        if (popSlots[j].jobno === parseInt(jobId)) {
          reqTime.push(popSlots[j].time);
    ...
    if (reqTime.length > 0) {
      for (let i = 0; i < bookedSlots.length; i++) {
        const time = bookedSlots[i].time;
        const allMatch = reqTime[0].every(
          (value, index) => value === time[index]
        );
        if (allMatch) {
          console.log("id", bookedSlots[i]._id);
          deleteSlots(bookedSlots[i]._id);
          break;
        }
      }
    }
      ...
      <div class="d-flex justify-content-center my-5 ">
        ...
        popSlots.find((slot) => slot.jobno === parseInt(jobId))
          ...
          <button
            ...
            type="submit"
            onClick={() => {
              updatePopSlot(jobId, "acceptedFin");
              addNotification(
                studentInfo.institute_emailId,
                studentInfo.job_no,
                true,
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    addRejnMessage(jobId, "", reason.facultyIncharge);
                    updatePopSlot(jobId, "rejectedFin");
                    deleteTimeSlots(jobId);
                    deleteMe();
                    setShowModal(false);
                    navigate(`/combinedf/${email}`);
                  }}
