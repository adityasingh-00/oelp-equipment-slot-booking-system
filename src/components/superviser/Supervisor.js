...
function OurPage({ Data, email }) {
  ...
  useEffect(() => {
    if (Data) {
      setData(Data.map((item) => ({ ...item, isVerified: item.isVerified })));
    }
  }, [Data]);
    ...
    setData((prevData) => {
      ...
      setNotification(
        `Person ${updatedData[index].name} has been ${
          updatedData[index].isVerified ? "verified" : "unverified"
        }!`
      );
      setTimeout(() => setNotification(""), 3000); // Notification disappears after 3 seconds
      navigate(`/studentDetails1/${email}/${jobId}`);
  ...
  function handleFormSubmit(event) {
    ...
    setData((prevData) => {
      const updatedData = [...prevData, { ...newMember, isVerified: false }];
        ...
        {data.map((item, index) => (
          <div className="L21" key={index}>
            ...
            <button
              ...
              className={
                item.isVerified ? "verified-button" : "non-verified-button"
              }
            >
              {item.isVerified ? <p>Verified</p> : <p>NonVerified</p>}
            </button>
